...
> Репозиторий на github - https://github.com/shamhi/top-search
...
Проект построен по принципу **Ports & Adapters** (гексагональная архитектура) — бизнес-логика изолирована от
инфраструктурных деталей, все внешние зависимости подключаются через интерфейсы-порты.

**Три слоя:**

| Слой           | Пакет                | Что содержит                                                                                                                                                                                                                                             | Зависит от                                                          |
|----------------|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------|
| **Ядро**       | `internal/core/`     | `domain/` — бизнес-сущности (`SearchQueryEvent`, `TrendingQuery`), `ports/` — интерфейсы (`TrendService`, `TrendRepository`, `TrendBroker`), `service/` — бизнес-логика (`Ingest`, `GetTop`, нормализация, дедупликация), `config/` — viper-конфигурация | **Ни от чего** (только стандартная библиотека + zap, protobuf-типы) |
| **Адаптеры**   | `internal/adapters/` | `grpc/` — gRPC-сервер + health check, `nats/` — JetStream consumer, `redis/` — ZSET-репозиторий, `metrics/` — Prometheus HTTP                                                                                                                            | `core/ports/` (интерфейсы, которые реализуют)                       |
| **Композиция** | `internal/app/`      | `provider.go` — DI-контейнер (ленивая инициализация адаптеров), `app.go` — жизненный цикл (сигналы ОС, graceful shutdown)                                                                                                                                | Все слои (единственная точка, где адаптеры связываются с ядром)     |
...
in-memory кеш мгновенно. При старте сервис загружает stop-слова из Redis.

Матчинг — **токенный**: запрос разбивается на слова (`strings.Fields`), stop-слово применяется к каждому токену и ко
всей строке целиком. Stop-слово `"samsung"` заблокирует и `"samsung"`, и `"телевизор samsung 55 qled"` — но не
заблокирует
`"samsung-galaxy"` (слитный токен).
Stop-слова применяются на этапе Ingest (блокировка записи) и на этапе GetTop (фильтрация выдачи).
...
| Stop-слова: токенный матчинг     | Блокирует составные запросы (`"купить samsung"`), а не только точное совпадение | `"samsung-galaxy"` (без пробела) пройдёт мимо; решается нормализацией дефисов |
...
### Мониторинг-стек (Prometheus + Grafana + NATS Exporter)

```bash
task monitor     # поднять стек
# или
make monitor
